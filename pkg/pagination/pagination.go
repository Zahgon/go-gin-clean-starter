...
func BindPagination(ctx echo.Context) PaginationRequest {
	...
	if perPageStr := ctx.QueryParam("per_page"); perPageStr != "" {
		if perPage, err := strconv.Atoi(perPageStr); err == nil && perPage > 0 && perPage <= 100 {
			pagination.PerPage = perPage
		}
	}

	pagination.Search = ctx.QueryParam("search")

	pagination.Sort = ctx.QueryParam("sort")

	if order := ctx.QueryParam("order"); order == "desc" || order == "asc" {
