...
var (
	...
	// ErrConvertToMapString can not convert to map of strings
...
// MapFormWithTag maps a form source onto ptr using an arbitrary struct tag.
...
// setter tries to set a value while walking the fields of a struct.
...
type setOptions struct {
	...
	defaultValue    string
	// parser names the interface used to read the request and default values.
	parser string
}
...
func tryToSetValue(value reflect.Value, field reflect.StructField, setter setter, tag string) (bool, error) {
	...
	for len(opts) > 0 {
		...
		if k, v := head(opt, "="); k == "default" {
			...
			// convert semicolon-separated default values to csv-separated values
...
// BindUnmarshaler wraps the UnmarshalParam method.
...
func setWithProperType(val string, value reflect.Value, field reflect.StructField, opt setOptions) error {
	// required for parsing nested types like []MyId, where MyId is [12]byte
	if ok, err := trySetUsingParser(val, value, opt.parser); ok {
		return err
	} else if ok, err = trySetCustom(val, value); ok {
		return err
	...
	// string values are handed over untouched
